    ...
    <div className="flex flex-col h-full">
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-2">
          <div className="bg-foreground p-2 rounded-xl text-white">
            <Wallet size={20} />
          </div>
          <div>
            <h3 className="font-semibold text-lg leading-tight">Agent Portfolio</h3>
            <p className="text-xs text-muted">Real-time Balance</p>
          </div>
        </div>
        <div className="text-right">
          <p className={`text-sm font-medium flex items-center justify-end gap-1 ${isPositive ? 'text-success' : 'text-red-500'}`}>
      ...
      <div className="flex-1 flex flex-col justify-end">
        <h4 className="text-xs font-semibold text-muted uppercase tracking-wider mb-4">Asset Allocation</h4>
        
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center text-accent font-bold text-xs">M</div>
              <div>
                <p className="font-medium text-sm">MONAD</p>
                <p className="text-xs text-muted">{baseBalance.toFixed(4)} M</p>
              </div>
            </div>
            <div className="text-right">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted">{quoteBalance.toFixed(2)} USDC</p>
              </div>
            </div>
            <div className="text-right">
        ...
        <div className="mt-6 pt-6 border-t border-border grid grid-cols-3 gap-2 text-center">
          <div>
            <p className="text-[10px] text-muted uppercase tracking-widest mb-1">Trades</p>
            <p className="text-sm font-medium font-mono">{state?.recent_trades?.length || 0}</p>
          </div>
          <div>
            <p className="text-[10px] text-muted uppercase tracking-widest mb-1">Win Rate</p>
            <p className="text-sm font-medium font-mono">{state?.computed?.winRate || 0}%</p>
          </div>
          <div>
            <p className="text-[10px] text-muted uppercase tracking-widest mb-1">Avg Conf</p>
